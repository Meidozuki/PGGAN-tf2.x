...
def make_dataset(x,batch_size,repeat=None):
    dataset=tf.data.Dataset.from_tensor_slices(x)
    dataset=dataset.shuffle(2048).batch(batch_size,drop_remainder=True).repeat(repeat).prefetch(tf.data.experimental.AUTOTUNE)
    return dataset
    ...
    for i in range(loop_range):
        ...
        if predict:
            ...
            if pred.shape[-1] != 1:
                ...
                title="validity=%.3f" % (float(pred[i]))
            #标签部分
            if y is None:
                ax.set_title(title)
            else:
